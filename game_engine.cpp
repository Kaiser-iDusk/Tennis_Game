...
global_variable float ball_v_x = 250.f, ball_v_y = 0.f;

global_variable float scale = 1.01f;
...
static void simulate_game(struct Input* input, float dt) {
    ...
    if (is_down(BUTTON_UP)) player_acc_y += 750.f;
    if (is_down(BUTTON_DOWN)) player_acc_y -= 750.f;
